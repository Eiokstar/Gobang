...
public class ActionListener implements java.awt.event.ActionListener {
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        switch (command) {
            case MODE_PVP -> selectHumanVsHuman();
            case MODE_AI, MODE_AI_LEGACY -> selectHumanVsAI();
            case MODE_ONLINE -> selectOnlineMode();
            case DIFFICULTY_EASY -> changeAIDifficulty(Game.Difficulty.EASY);
            case DIFFICULTY_MEDIUM -> changeAIDifficulty(Game.Difficulty.MEDIUM);
            case DIFFICULTY_HARD -> changeAIDifficulty(Game.Difficulty.HARD);
            case "重新" -> resetGame();
            case "開始" -> startGame();
            case "認輸" -> surrender();
            case "悔棋" -> undoMove();
            case "確定" -> confirmDialog();
            case "回復棋子" -> restorePieces();
            case "先手" -> chooseFirstHand();
            case "後手" -> chooseBackHand();
            default -> {
            }
        }
    }
    ...
    private void selectOnlineMode() {
        ...
        int result = JOptionPane.showOptionDialog(UI.windows, "請選擇聯機方式", "聯機對戰",
    ...
    private Game.Difficulty promptDifficultySelection() {
        ...
        if (defaultIndex < 0 || defaultIndex >= options.length) defaultIndex = 0;

        int result = JOptionPane.showOptionDialog(UI.windows, "請選擇AI難度", "選擇難度",
                JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, labels, labels[defaultIndex]);

        if (result < 0 || result >= options.length) return null;
    ...
    private void startGame() {
        ...
        if (!game.Started) {
            if (game.vsComputerMode && game.backHand) game.changeCurrentPlayer();
